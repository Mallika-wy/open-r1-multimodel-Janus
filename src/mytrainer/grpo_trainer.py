...
# conditional import，根据是否安装了特定的库来决定是否导入相关的模块
# 这种做法通常用于处理可选依赖（optional dependencies），使得代码在相关库未安装时仍然可以运行，而不会因为导入错误而中断
if is_peft_available(): # peft库 用于参数高效微调
    from peft import PeftConfig, get_peft_model

if is_wandb_available(): # wandb库 用于实验跟踪和模型管理
    import wandb
...
class JanusGRPOTrainer(Trainer):
    def __init__(
        self,
        model: Union[str, PreTrainedModel],
        reward_funcs: Union[RewardFunc, list[RewardFunc]],
        args: GRPOConfig = None,
        train_dataset: Optional[Union[Dataset,IterableDataset]] = None, # IterableDataset 允许数据集被迭代，而不是一次性加载到内存中
        ...
        if isinstance(model, str):
            model_id = model
            torch_dtype = model_init_kwargs.get("rotch_dtype")
        ...
        # Reference model 
        # 其的作用是提供一个固定的策略，用于计算当前策略的基准。
        # 这个基准用于确定策略更新的方向和幅度，确保更新是稳定且有益的。
        ...
        # Processing class
        # 处理输入数据，确保模型能够正确地接收和理解输入的文本或序列数据
